...
#### Serviço
 ...
 - Opções: 
   ...
   - `"Manutenção da Consignação"`
   - `"Consignação de Parcelas"`
   - `"Glosa da Consignação (INSS)"`
   - `"Consulta de Tributos a Pagar DETRAN (SP, RS e MG) com RENAVAM"`
   - `"Debêntures"`
   ...
   - `"Fundos de investimentos ou vendor"`
   - `"Vendor a Termo"`
   - `"Pagamento sinistros segurados"`
   - `"Consulta e Agenda Boletos de Cobrança"`
   - `"Pagamento despesas viajante em trânsito"`
   - `"Pagamento autorizado"`
   - `"Pagamento credenciados"`
   ...
   - `"Custódia de Cheques"`
   - `"Gestão de Caixa"`
   - `"Consulta/Informação Margem"`
   - `"Averbação da Consignação/Retenção"`
...
#### Forma de Pagamento
 ...
 - Opções: 
   ...
   - `"GARE SP ITCMD"`
   - `"IPVA"`
   - `"Licenciamento SP, RS e MG – Pagamento com o RENAVAM"`
   - `"DPVAT"`
   - `"Pagamento de títulos em cobrança do próprio banco"`
   - `"Pagamento de títulos em cobrança em outros bancos"`
   - `"Nota fiscal – liquidação eletrônica"`
   - `"FGTS"`
   - `"Extrato de Conta Corrente"`
   - `"TED – outro titular"`
   - `"TED – mesmo titular"`
   - `"TED para Transferência de Conta Investimento"`
   - `"Pix transferência"`
   - `"Pix qr-code"`
   - `"Débito em conta corrente – recebimento"`
   - `"Cartão salário"`
   - `"Extrato para Gestão de Caixa"`
   - `"Pagamento de Guia de Depósito Judicial"`
   - `"Depósito Judicial em Poupança"`
   - `"Extrato de Conta Investimento"`
   - `"GNRE e tributos com código de barras"`
   - `"Pagamento de Concessionárias ou Cadastro de favorecidos"`
...
#### Digito Verificador do Recebedor
